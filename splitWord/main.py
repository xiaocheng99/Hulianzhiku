...
def excute_data(res, workbook, worksheetName):
    ...
    i = 2  # 从第二行开始写入数据
    for re in res:
        seg_list = jieba.cut_for_search(re[0])  # 切换模式
...
if __name__ == '__main__':
    ...
    workbook = xw.Workbook('./accuracy.xlsx')  # 创建工作簿 传入文件地址
    sql = "SELECT `call_content`,`undertaking_department` FROM `work_order` where `undertaking_department` in ('公安局')"
